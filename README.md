...
- **Plugin hot reload (HMR)**: runtime plugin loading from jars (SPI discovery + dedicated `PluginClassLoader`), jar hot-swap on change (atomic replacement + rollback on failure), complete class unloading; generic file watching via `Hmr.watch(path, callback)` (the `Include` plugin owns its own config file through it), two-way config sync (runtime changes are written back to the file, file edits win conflicts — `commit(EntryChange)` + journal), plus a convenience config reload mode (see the `examples/hmr-app` demo)
